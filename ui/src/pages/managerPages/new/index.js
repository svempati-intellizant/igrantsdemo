...
function ManagerLanding() {
  ...
  useEffect(() => {
    ...
    if (landView === "agency") {
      ...
      if (grantList) {
        ...
        const cur = grantList.filter(
          (x) => x.agency === pathLocation.split("/")[3].replaceAll("%20", " ")
        )[0];
        const programListData = [
          ...new Set(grantList.filter((x) => x.agency === cur.agency).map((x) => x.program)),
        ];
  ...
  useEffect(() => {
    if (grantList) {
      ...
      switch (viewMode) {
        ...
        case "program":
          setAgencyProgram(programListData);
          setProgramGrants([empty, ...grantListData]);
          break;
        default:
          break;
      }
  ...
  const grantChangeHandler = (event, newvalue) => {
    ...
    if (newvalue != null && newvalue.grant_id != "") {
  ...
  return (
    <ManagerLandingWrapper>
            ...
            <Paper elevation={3}>
              <div className="mainToggler">
                <Grid container spacing={3} className="customGridTop" alignItems="center">
                  ...
                  {/* ------- Grant Name SELECTOR -------- */}
                  <Grid item xs={12} md={4} className="customGridItemTop">
                    {currentView != "detailed" && (
                      <>
                        <Autocomplete
                          name="grant Name"
      ...
      ---------------------------------------------------------------------------- */}
          ...
          {currentView != "detailed" && viewMode === "grant" && (
            <Paper elevation={2} style={{ marginTop: "10px" }}>
              <div className="mainToggler">
                {/* -------------------------- pickers ----------------------------------*/}

                <Grid container spacing={3} className="customGridTop" justify="flex-end">
                  {/* ------- AGENCY SELECTOR -------- */}
