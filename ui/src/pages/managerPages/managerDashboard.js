...
const ManagerDashboard = () => {
  ...
  const ApprovedFundDataValueParser = (type, field) => {
    ...
    if (field === "label") {
      return dashData[type]
        .sort(
          (a, b) =>
            new Date(a.grant_from_date).getMonth() -
            new Date(b.grant_from_date).getMonth()
        )
        .map((x) => {
  ...
  const ApprovedFundAmountCalculator = (type) => {
    if (!dashData) return [];
    return dashData[type].reduce(
      (total, obj) => obj.grant_authorized + total,
      0
    );
  };
  ...
  const periodOfPerformance = () => {
    ...
    return {
      ...
      authorized_grant: dashData.period_of_performance.map(
        (x) => x.grant_authorized
      ),
      total_disbursement: dashData.period_of_performance.map(
        (x) => x.total_disbursed_fund
      ),
    };
  };
    ...
    datasets: [
      {
        ...
        data: activitiesUnallowedValueParser("answerOfActivitiesUnallowed")
          .data,
        barPercentage: 0.3,
  ...
  const allowableCostPrinciples = {
    ...
    datasets: [
      {
        label: "value",
        data: activitiesUnallowedValueParser("answerOfAllowableCostPrinciples")
          .data,
        barPercentage: 0.3,
  ...
  const procurementSuspensionData = {
    ...
    datasets: [
      {
        label: "value",
        data: activitiesUnallowedValueParser(
          "answerOfProcurmentSuspensionDebarment"
        ).data,
        barPercentage: 0.3,
        backgroundColor: ["#EC6262", "#83D6B7"],
        borderColor: ["#EC6262", "#83D6B7"],
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={6} md={4}>
            <Paper elevation={10} className="customPaper">
              <div className="valueHolder">
                <p>Approved Fund</p>
                <h1>$ {ApprovedFundAmountCalculator("approvedFund")}</h1>
              </div>
          ...
          <Grid item xs={6} md={4}>
            <Paper elevation={10} className="customPaper">
              <div className="valueHolder">
                ...
                <h1>
                  ${" "}
                  {disbursedFundDataValueParser().reduce(
          ...
          <Grid item xs={6} md={4}>
            <Paper elevation={10} className="customPaper">
              <div className="valueHolder">
                ...
                <h1>
                  ${" "}
                  {upcomingDisbursementValueParser().reduce((a, b) => a + b, 0)}
                </h1>
              </div>
            ...
            <Paper elevation={10} className="customPaper">
              <div className="valueHolder">
                ...
                <div className="customLegend">
                  <div>
                    ...
                    ></span>{" "}
                    <span>
                      {acceptedApplicant.datasets[0].data[0]} Accepted
                    </span>
                  </div>
                  <div>
                    <span
                    ...
                    ></span>{" "}
                    <span>
                      {acceptedApplicant.datasets[0].data[1]} Rejected
                    </span>
                  </div>
                </div>
              </div>
              <div className="customSemiDoughnutHolder">
                <Doughnut
                  ...
                  options={portFolioAtRiskOptions}
                />
              </div>
            </Paper>
          </Grid>
          ...
          <Grid item xs={6} md={4}>
            <Paper elevation={10} className="customPaper">
              ...
              <Bar
                data={approvedVsDisbursedData}
                options={approvedVsDisbursedOptions}
              />
            </Paper>
          </Grid>
          {/**
